...
meta732 <- read_csv("~/Github/pvdiv-climate-gwas/data/Metadata_2020-02.csv")
meta732 <- meta732 %>%
  add_row(PLANT_ID = "AP13", LIBRARY = "REF", Genetic_subpop_50per = "Texas", Genetic_subpop_95per = "Texas", Ecotype = "Texas", Texas_Q = 1, Eastcoast_Q = 0, Midwest_DA = 0, LIB_GROWN = "Y")
...
phenotypes_long_2019 <- phenotypes_wide_2019 %>%
  pivot_longer(cols = BRKG_BIOMASS:TMPL_TC_EOS, names_to = "SITE_PHE", values_to = "Value") %>%
  separate(col = "SITE_PHE", into = c("SITE", "_", "PHE"), sep = c(4,5), remove = FALSE) %>%
  dplyr::select(-"_")
```
...
phenotypes_long_2019 <- phenotypes_2019 %>%
  ...
  pivot_longer(cols = FRZ_DMG:T_EM_FL, names_to = "PHE", values_to = "Value") %>%
  filter(!is.na(Value)) %>%
...
```{r}
phenotypes_long_2019 %>%
  filter(grepl("TC_EOS", PHE) | grepl("HT_PAN_EOS", PHE) | grepl("BIOMASS", PHE) | grepl("MOIST", PHE)) %>%
  ggplot(aes(x = Value)) +
  geom_histogram(aes(fill = SITE)) + 
  facet_grid(SITE~PHE, scales = "free") +
  scale_fill_viridis(discrete = TRUE, end = 0.95, direction = -1) +
...
phenotypes_long_2019 %>%
  filter(grepl("MOIST", PHE)) %>% arrange(desc(Value))
phenotypes_long_2019 %>%
  filter(grepl("BIOMASS", PHE)) %>% arrange(desc(Value))
```

SITE  PLOT_GL   PLOT_LC  ACC  PLANT_ID  PHE  Value
KBSM	M5710	5710	J476	J476.C	MOISTURE	-1154.137931	NA	NA	
KBSM	M4412	4412	J024	J024.A	MOISTURE	-195.065147
KBSM	M6607	6607	J646	J646.A	MOISTURE	973.34815	

KBSM	M5710	5710	J476	J476.C	BIOMASS	19439.138	
...
```{r}
meta732$Genetic_subpop_50per <- factor(meta732$Genetic_subpop_50per, levels = c("Texas", "Eastcoast", "Midwest"))

phenotypes_long_2019 %>%
  filter(grepl("BIOMASS", PHE)) %>%
  left_join(meta732, by = "PLANT_ID") %>%
  filter(!(Genetic_subpop_50per %in% c("Admixed", "NA")), !is.na(Genetic_subpop_50per)) %>%
  ggplot(aes(x = Value)) +
  geom_histogram(aes(fill = Genetic_subpop_50per), binwidth = 500) + 
  facet_grid(SITE~PHE, scales = "free_y") +
  scale_fill_viridis(discrete = TRUE, end = 0.8, direction = -1) +
  theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1)) + xlab("Value") + ylab("Count of Individuals") +
  geom_vline(xintercept = 2800, linetype = 3) +
  scale_y_continuous(breaks = c(10, 50, 100))

save_plot("2019_Phenotypic_Data_by_Site_and_Genetic_Subpop_scale_2020-02-14.png", plot = last_plot(), base_height = 7, base_asp = 0.4)
