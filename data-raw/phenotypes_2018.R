## code to prepare `phenotypes_2018` dataset goes here
library(tidyverse)
library(XLConnect)
library(rlang)
library(switchgrassGWAS)

## Load all the phenotypic data from 2018 ## --------------
## -------------------------------------------
...
wb_CLMB2018 <- loadWorkbook(file.path("C:", "Users", "alice", "Dropbox",
                                      paste0("GWAS DOE Switchgrass, ",
                                             "Consolidated Data"),
...
KBSM2018 <- read_csv(file.path("C:", "Users", "alice", "Dropbox",
                               paste0("GWAS DOE Switchgrass, ",
                                      "Consolidated Data"),
                               "Individual Site Files",
                               "KBSM_GWAS_2018_Rust Data_Final.csv"),
                     col_names = TRUE)
KING2018 <- read_csv(file = file.path("C:", "Users", "alice", "OneDrive",
                                      "Juenger Projects", "SwitchgrassGWAS",
                                      "Metadata_and_Phenotypes",
                                      "Armyworm damage KING 2018.csv"),
                     col_names = TRUE)
TMPL2018 <- read_csv(file = file.path("C:", "Users", "alice", "OneDrive",
                                      "Juenger Projects", "SwitchgrassGWAS",
                                      "Metadata_and_Phenotypes",
...
wb_PKLE2018 <- loadWorkbook(file.path("C:", "Users", "alice", "OneDrive",
                                      "Juenger Projects", "SwitchgrassGWAS",
                                      "Metadata_and_Phenotypes",
...
## -------------------------------------------
# Join all the sources of phenotypic data from 2018 together.
...
# Join the phenotype data from 2018 to the metadata
#
pvdiv_784g <- metadata %>%
...
pvdiv_784g <- pvdiv_784g %>%
  mutate(TC_EOS_2018 = as.numeric(TC_EOS_2018),
         ...
         GR1 = as.numeric(GR1), # These are data from 2019
         GR50 = as.numeric(GR50), # These are data from 2019
         GR100 = as.numeric(GR100), # These are data from 2019
...
#NB: No outlier removal was done
# This step puts the phenotypes in wider format so each column
# has one phenotype at one site.
phenotypes_2018 <- phenotypes %>%
...
save(phenotypes_2018, file = "data/phenotypes_2018.rda")
