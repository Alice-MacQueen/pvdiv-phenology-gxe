...
phenotypes <- readRDS(here("data/Julian_date_GDD_and_rainfall_phenotypes_4wcr_and_gwas.rds"))
h2gr <- read_csv(here("analysis", "heritability", "kinship_subpop", "h2_df_GR50_related_functions_of_weather_v1.csv"))
h2fl <- read_csv(here("analysis", "heritability", "kinship_subpop", "h2_df_FL50_related_functions_of_weather_v1.csv"))
h2gr_gxe <- read_csv(here("analysis", "heritability", "kinship_subpop", "gxe_and_h2_df_GR50_related_functions_of_weather_v2.csv"))
h2fl_gxe <- read_csv(here("analysis", "heritability", "kinship_subpop", "gxe_and_h2_df_FL50_related_functions_of_weather_v1.csv"))
...
h2fl <- h2fl %>%
  full_join(h2fl_gxe)
```
as well as the code for any new analyses that I'm running to do new writeup for the manuscript v0.3.
...
save_plot(filename = here("manuscript", "revision2", "intermediate_files", "Map_subpops_and_Ecotypes.png"), plot = last_plot(), base_height = 3)
?geom_point
```
...
pheno_corr <- plot_grid(panc, panf, panb, pane, pana, pand, nrow = 3)
save_plot(filename = here("manuscript", "revision2", "intermediate_files", "Phenotypic_correlations_GR50_FL50_2.svg"), plot = pheno_corr, base_width = 3.5, base_height = 4.5)
```
...
Use code starting ~on line 966 to generate this.
```{r}
site_plot <- c("All", "Texas", "North")
...
save_plot("../manuscript/plots/Var_Comp_greenup_site_groups.svg", plot = gr_bar_gxe, base_height = 4, base_asp = 1.5)
```
...
phe_fl <- c("FL50", "dyln_fl50", "dyln_change_sec", "cgdd_12c_gr2fl",
           ...
           "crain_7d")
phe_gr <- c("GR50", "DYLN", "cgdd_12c_jan2gr", "cgdd_8c_jan2gr", "tave_10d",
            "tave_18d", "tave_5d", "cgdd_12c_10d",
            "cgdd_12c_5d", "cgdd_12c_18d", "tmax_18d", 
            "tmin_18d",  "tmax_10d", "tmin_10d",
            "tmax_5d", "tmin_5d")
...
phe_gr <- c("GR50", "DYLN", "cgdd_12c_jan2gr", "tave_18d", "cgdd_12c_18d",
            "tave_10d", "cgdd_12c_10d", "tave_5d", "cgdd_12c_5d")
phe_dfs <- list(phe_gr_df, phe_fl_df)
phe_lists <- list(phe_gr, phe_fl)
...
# ---- moved to new doc: Analysis_manuscript_mash_GxE.Rmd ----
...
j=1 # phe
k=1 # subpop
phe_suffix <- paste(phe_single[j], names(subpop_v)[k], 
                    sep = "_")
gwas_rds <- pvdiv_results_in_folder(path = file.path(outputdir,
                                                       names(set_v)[j]), 
                                      pattern = paste0("GWAS_datatable*.*",
...
May want to rerun pvdiv_bigsnp2mashr function so that I'm looking at GR & FL top 1000 SNPs separately.
...
for(j in seq_along(set_v)){
  anno_list <- list()
  annodir <- file.path(workingdir, "analysis", "gwas", "BLUPs",
                       names(set_v)[j])
  anno_v <- pvdiv_results_in_folder(path = annodir, pattern = "Annotation")

  for(i in seq_along(anno_v)){
  annos <- readRDS(file.path(annodir, anno_v[i]))
  if(names(set_v)[j] != "single_sites"){
  anno_info <- anno_v[i] %>% enframe(name = NULL, value = "File") %>%
    mutate(phe = case_when(grepl("dyln_fl50", File) ~ "dyln_fl50",
           ...
           sites = case_when(grepl("eight_sites", File) ~ "eight_sites",
                             grepl("north_sites", File) ~ "north_sites",
                             grepl("tx_sites", File) ~ "tx_sites"),
           subpop = case_when(grepl("8\\.8M", File) ~ "Midwest",
                              grepl("10\\.3M", File) ~ "Gulf",
                              grepl("12\\.1M", File) ~ "Gulf_and_Midwest"))
  } else {
    anno_info <- anno_v[i] %>% enframe(name = NULL, value = "File") %>%
    mutate(phe = case_when(grepl("dyln_fl50", File) ~ "dyln_fl50",
                           grepl("cgdd_12c_gr2fl", File) ~ "cgdd_12c_gr2fl",
                           ...
                           grepl("crain_1d", File) ~ "crain_1d",
                           grepl("cgdd_12c_10d", File) ~ "cgdd_12c_10d"),
           sites = case_when(grepl("TX1", File) ~ "TX1",
                             grepl("TX2", File) ~ "TX2",
                             grepl("TX3", File) ~ "TX3",
                             grepl("OK", File) ~ "OK",
                             grepl("MO", File) ~ "MO",
  ...
  anno_list[[i]] <- annos$top500SNPs_within50000bp %>%
    arrange(QUERYID, `distance from gene`) %>%
    group_by(QUERYID) %>%
    slice(1) %>%
    ungroup() %>%
    mutate(POS_Mb = floor(POS/10000)/100,
           phe = anno_info$phe,
           sites = anno_info$sites,
           subpop = anno_info$subpop) %>%
    select(sites, subpop, phe, CHR, POS_Mb, `Gene ID`, `distance from gene`, `FDR Adjusted p value`, `Arabidopsis thaliana homolog`:`A. thaliana gene annotation`, `Rice gene annotation`, `Rice homolog`)
  }
  
  anno_df <- anno_list[[1]]
  for(i in seq_along(anno_list)[-1]){
    anno_df <- anno_df %>%
      full_join(anno_list[[i]])
  }
  anno_df %>%
    arrange(CHR, POS_Mb) %>%
    filter(`FDR Adjusted p value` <= 0.1) %>%
write_csv(., path = file.path(workingdir, "analysis", "gwas", "BLUPs",
                                    paste0(names(set_v)[j],     
                                           "_top_500_SNPs_above_FDR",
                                           "_closest_annotations.csv")))
}
```
Taylor et al 2019, Crop Science: switchgrass GWAS
Grabowski et al 2017, New Phytologist - GWAS on ~509 individuals on heading date in New York
Switchgrass GWAS at two locations: Frontiers in Plant Science
Pavir.8NG050900: Pseudo Response Regulator 5 (PRR5) - important in flowering date
Flowering locus T (on Chromosome 5N). FKF1, PRR5 COP1 GASA8 P450 - differentially expressed flowering genes between early and late flowering individuals.
 
Pavir.2NG448600: PRR5
Pavir.5KG044000 (FT)
Pavir.2NG126000 (PRR7)
Pavir.
```{r}
annos_dyln$top500SNPs_within50000bp %>%
  filter(CHR == "Chr05N" & region_start > 64700000)

annos_df %>%
  filter(CHR == "Chr05N" & POS_Mb > 64.6)

annos_dyln$top500SNPs_within50000bp %>%
  filter(CHR == "Chr05N" & region_start < 3000000 & region_start > 2000000) %>%
  select(`Gene ID`, `distance from gene`, `Arabidopsis thaliana homolog`, `Rice homolog`)
```
	Pavir.5NG109746 (18.7kb), 	AT1G49200 RING/U-box superfamily protein
		LOC_Os01g64620.1 highly expressed hypocotyl, silique
	Pavir.5NG109743 (22.5kb), 	AT2G22590 UDP-Glycosyltransferase superfamily pt.
	LOC_Os01g64540.1  highly expressed in mature flower petals
	Pavir.5NG109744 (17.9kb), 		AT2G35930 (PUB23) plant U-box 23
		LOC_Os01g64570.1 highly expressed in senescent leaf petiole; negative regulation of drought tolerance
	Pavir.5NG109745 (4.1kb)     AT2G28510 Dof-type zinc finger DNA-binding family protein  highly expressed in the carpel of the young flower & in seeds during germination day 1; promotes leaf senescence in arabidopsis
		LOC_Os01g64590.1
## ---- #### ---- ####
```{r}
workingdir <- file.path("~", "Github", "pvdiv-phenology-gxe")
set_v <- list(eight_sites = "eight_sites", tx_sites = "tx_sites", north_sites = "north_sites", single_sites = "!sites")
j = 1
annos_df <- read_csv(file = file.path(workingdir, "analysis", "gwas", "BLUPs",
                                    paste0(names(set_v)[j],     
                                           "_top_500_SNPs_above_FDR",
                                           "_closest_annotations.csv")))
for(j in seq_along(set_v)[-1]){
...
annos_plot <- annos_df %>% 
  ...
  select(n_SNPs, n_Mb, everything())
annos_plot1 <- annos_plot %>% group_by(CHR, POS_bin_hi, POS_Mb) %>% tally() %>%
  mutate(POS_lo = POS_Mb*1000000,
         POS_hi = POS_Mb*1000000+10000)
```

dyln, fl50, cgdd eight sites gulf & midwest, north sites gulf & midwest cgdd & dyln, tx crain gr2fl, dyln, cgdd
```{r}
qtldf %>% arrange(CHR, Pos_Mb_lo) %>%
  #filter(lodcolumn != "dyln_change_sec") %>%
  ggplot(aes(x = Pos_Mb_lo, y = lod)) +
  geom_jitter(aes(color = lodcolumn), height = 2) +
  geom_errorbarh(aes(y = lod, xmin = Pos_Mb_lo, xmax = Pos_Mb_hi, color = lodcolumn), size = 1.5) +
  facet_wrap(~CHR) + scale_color_manual(values = c("magenta", "yellow", "darkgreen", "blue")) + theme(legend.position = c(0.9, 0.15), panel.background = element_rect(fill = "grey90")) + xlim(c(0, 70)) + geom_vline(xintercept = 0)
```

3 regions with the most 'independent' hits: Chr03K 50.84, Chr03N 31.73, Chr05N 64.86
3N isn't potentially confirmed by any QTL. 3K doesn't overlap. 
5N does!All four traits for QTL (dyln much higher lod).
Then: Chr03K 38.23, Chr04K 8.07, Chr05N 2.79, Chr09N 69.33
3K ~ 17 - 26. Not really.  4K 2.28-5.5 & 9.92 - 13.93, so definitely close/overlaps. 5N overlaps. 9N does not.

So 5N (late), 5N (early), and 4K have overlaps thus far. Very strong result.

Then: 
Chr01K 56.75 (no), Chr02K 4.25 (yes), 3K 38.23, 38.27, 38.35, 40.87 (no), Chr04N 42.92 (no), Chr05N 49.05 (very close ~ 44.5 - 46.6), Chr07K 33.33 (very close 34.82), Chr08N 28.5 (yes), Chr08N 43.77 (no), Chr09N 21.45 26.06 30.04 (no but 21 is close: 6.88 - 17.31)

So for 7 regions sig above FDR in >=8 subsets, 3 overlap, and overlap the 3 of the 4 biggest QTL.
For the next 14 regions sig above FDR in 7 or more subsets, 2 overlap and 3 are very close. 
So of the 12-13 QTL, we find overlap for 3 of the 4 biggest, and 2-5 of the remainder. We don't find big hits in GWAS on 2N.

Biggest QTL: 2N, 4K, 5N (x2-3) (4-5)
Other QTL: 2K (x2), 3K, 7K, 7N, 8N, 9K, 9N (8)

nothing on 1K, 1N, 3N, 4N, 5K, 6K, 6N, 8K. Nothing!!

I think this is good. And can write up this section focusing on this.
```{r}
...
facet grid - rows are three sets of sites, colors are phenotypes, shapes are subpopulations. Then I can plot -log10 p values on the y axis and Chr/Pos on the x axis. 
```{r}
workingdir <- file.path("~", "Github", "pvdiv-phenology-gxe")
subpop_v <- list(Gulf_and_Midwest = c("Gulf", "Midwest"), Midwest = "Midwest", Gulf = "Gulf")
set_v <- list(eight_sites = "eight_sites", tx_sites = "tx_sites", north_sites = "north_sites", single_sites = "!sites")
...
for(i in seq_along(subpop_v)){
  for(j in 1:3){
    if(i == 1 & j ==1){
      logpdf <- readRDS(file.path(workingdir, "analysis", "gwas", "BLUPs",
                                  names(set_v)[j],
...
log10p_plot %>%
  ...
  geom_vline(data = annos_plot1, aes(xintercept = POS), color = "grey70", 
             size = 1.1) +
  geom_point(aes(color = phe_name, shape = subpop_name)) +
  facet_grid(site ~ CHR, scales = "free_x", space = "free_x", switch = "x") +
      theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
              legend.position = "top",
            panel.background = element_rect(fill = "grey85")) +
            labs(x = "Chromosome", y = "-log10(p value)") +
            scale_x_continuous(breaks = c(20000000, 40000000, 60000000, 80000000, 100000000), labels = c("20 Mb", "40 Mb", "60 Mb", "80 Mb", ""), expand = c(0.4, 0.4)) +
      scale_color_viridis(discrete = TRUE, end = 0.93,
                          option = "B") +
      scale_shape_manual(values = c(6, 4, 1, 4, 16, 4, 17, 18, 14, 3, 4)) +
  ylim(c(6, 15)) + geom_hline(yintercept = c(8.3), linetype = 2)
save_plot(paste0("../manuscript/plots/", "G_BLUP_Combined_Manhattans_fl_phe_subpops_sites.png"), 
              base_height = 4.5, base_asp = 3.5, plot = last_plot())
...
k_subgenome <- log10p_plot %>%
  filter(grepl("K", CHR)) %>%
  ggplot(aes(x = POS, y = log10p)) +
  geom_vline(data = filter(annos_plot1, grepl("K", CHR)),
             aes(xintercept = POS), color = "grey70", size = 1.1) +
  geom_point(aes(color = phe_name, shape = subpop_name), size = 0.7, stroke = 0.8) +
  geom_blank(data = filter(log10p_lim, grepl("K", CHR))) +
  facet_grid(site ~ CHR, scales = "free_x", space = "free_x", switch = "x") +
      theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), 
              legend.position = "top",
            panel.background = element_rect(fill = "grey85"), panel.spacing.x = unit(0, 'cm')) +
            labs(x = "Chromosome", y = "-log10(p value)") +
            scale_x_continuous(breaks = c(20000000, 40000000, 60000000, 80000000), labels = c("20 Mb", "40 Mb", "60 Mb", "80 Mb"), expand = c(0.1, 0.1)) +
      scale_color_viridis(discrete = TRUE, end = 0.88,
                          option = "B") +
...
n_subgenome <- log10p_plot %>%
  filter(grepl("N", CHR)) %>%
  ggplot(aes(x = POS, y = log10p)) +
  geom_vline(data = filter(annos_plot1, grepl("N", CHR)),
             aes(xintercept = POS), color = "grey70", size = 1.1) +
  geom_point(aes(color = phe_name, shape = subpop_name), size = 0.7, stroke = 0.8) +
  geom_blank(data = filter(log10p_lim, grepl("N", CHR))) +
  facet_grid(site ~ CHR, scales = "free_x", space = "free_x", switch = "x") +
      theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), 
              legend.position = "none",
            panel.background = element_rect(fill = "grey85"), panel.spacing.x = unit(0, 'cm')) +
      ...
      scale_color_viridis(discrete = TRUE, end = 0.88,
                          option = "B") +
      scale_shape_manual(values = c(1, 4, 6, 4, 16, 4, 17, 18, 14, 3, 4)) +
  ylim(c(6, 15)) + geom_hline(yintercept = c(8.3), linetype = 2) + labs(subtitle = "N subgenome", x = "", y = "-log10(p value)")

both_subgenomes <- plot_grid(k_subgenome, n_subgenome, nrow = 2, rel_heights = c(0.52, 0.47))
save_plot(paste0("../manuscript/plots/", "G_BLUP_Combined_Manhattans_fl_phe_subpops_sites_by_subgenome.svg"), 
              base_height = 7.5, base_width = 6.5, plot = both_subgenomes)
```

## Manhattan QTL windows
