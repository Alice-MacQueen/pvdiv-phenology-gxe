...
gr_ten_sites <- readRDS(file = here("data",
                                    "Weather_related_phe_GR50_3yr.rds"))
fl_ten_sites <- readRDS(here("data", "Weather_related_phe_FL50_3yr.rds"))
...
for(i in seq_along(phe_hyp)){
  for(k in seq_along(subpop_v2)){
    ...
    U_hyp_2019_gr <- list.prepend(U_hyp_2019_gr, cor_phe)
    names(U_hyp_2019_gr)[1] <- paste0(phe_hyp[i], "_", names(subpop_v2)[k])
  }
}

saveRDS(U_hyp_2019, file = here("data", "P_covar_matrices_flowering_2019_8sites.rds"))
saveRDS(U_hyp_2019_gr, file = here("data", "P_covar_matrices_greenup_2019_8sites.rds"))
```
