...
mash_plot_pairwise_sharing(corrmatrix = gxe$S_all)
max(gxe$S_DS)
min(gxe$S_DS)
```
...
sum(gxe$S_AP)/78411 # 510
sum(gxe$S_DS) # 3188
sum(gxe$S_2_no) # 118913
sum(gxe$S_all) # 130939
sum(gxe$S_CN) # 11516

(sum(gxe$S_AP)/sum(gxe$S_all))*length(get_significant_results(m))
(sum(gxe$S_CN)/sum(gxe$S_all))*length(get_significant_results(m))
(sum(gxe$S_2_no)/sum(gxe$S_all))*length(get_significant_results(m))
(sum(gxe$S_DS)/sum(gxe$S_all))*length(get_significant_results(m))
19780+85+1915+530
length(gxe$S_all)
130939/81
# 21781
# 
mash_plot_pairwise_sharing(corrmatrix = (gxe$S_AP/gxe$S_all))
max((gxe$S_AP/gxe$S_all))
max((gxe$S_CN/gxe$S_all))
max((gxe$S_2_no/gxe$S_all))
max((gxe$S_DS/gxe$S_all))
mean((gxe$S_AP/gxe$S_all))
mean((gxe$S_CN/gxe$S_all))
mean((gxe$S_2_no/gxe$S_all)) + mean((gxe$S_DS/gxe$S_all))
...
median(AP, na.rm = TRUE)
median(CN, na.rm = TRUE)
median(DS, na.rm = TRUE)
median(twono, na.rm = TRUE)
median(all, na.rm = TRUE)

sum(AP, na.rm = TRUE)/sum(diag(all))
sum(CN, na.rm = TRUE)/sum(diag(all))
sum(DS, na.rm = TRUE)/sum(diag(all)) + sum(twono, na.rm = TRUE)/sum(diag(all))
sum(all, na.rm = TRUE)/sum(diag(all))

sum(AP, na.rm = TRUE)/(sum(all, na.rm = TRUE))
#sum(CN, na.rm = TRUE)/(2*sum(all, na.rm = TRUE))
sum(DS, na.rm = TRUE)/(sum(all, na.rm = TRUE))
sum(twono, na.rm = TRUE)/(sum(all, na.rm = TRUE))
sum(all, na.rm = TRUE)/sum(all, na.rm = TRUE)
...
mean(AP, na.rm = TRUE)
mean(CN, na.rm = TRUE)
mean(DS + twono, na.rm = TRUE) 
mean(twono, na.rm = TRUE)
mean(all, na.rm = TRUE)
7+160+44+76
max(all, na.rm = TRUE)
120+159+7
...
Instead, the vast majority of SNPs have common effects across sites (
`r mean((gxe$S_2_no/gxe$S_all)) + mean((gxe$S_DS/gxe$S_all))`) or are conditionally neutral (`r mean((gxe$S_CN/gxe$S_all))`) ( e.g., having effects at some sites but not others) with only `r mean((gxe$S_AP/gxe$S_all))` showing sign change.
