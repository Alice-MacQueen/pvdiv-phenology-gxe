...
library(GGally)
pairplotFL <- mash_plot_pairwise_sharing(corrmatrix = pairwise_plot$corr_matrix[c(4,6,8,7,2,5,3,1),c(4,6,8,7,2,5,3,1)], reorder = FALSE)

corrplot1 <- ggcorr(data = NULL, cor_matrix = pairplotFL$corr_matrix, 
                       label = FALSE, label_alpha = TRUE,
                       label_size = 3, hjust = 0.95, vjust = 0.3,
                       layout.exp = 9, min_size = 0,
                       max_size = 3.5, limits = c(0,1), low = "#000077", mid = "#EEEEEE", high = "#FF5500")
save_plot("Pairwise_FL50_latitude_orange.svg", plot = corrplot1, base_height = 2.5, base_asp = 1.618)

ggcorr(data = NULL, cor_matrix = pairwise_plot2$corr_matrix[c(5,7,6,2,4,3,1),
                                                            c(5,7,6,2,4,3,1)], 
       label = FALSE, label_alpha = TRUE, label_size = 3, hjust = 0.95, 
       vjust = 0.3, layout.exp = 9, min_size = 0, max_size = 3.5, 
       limits = c(0,1), low = "#000077", mid = "#EEEEEE", high = "#FF5500")
save_plot("Pairwise_T_GR_FL_latitude_orange.svg", plot = last_plot(), base_height = 2.5, base_asp = 1.618)

ggcorr(data = NULL, cor_matrix = pairwise_plot3$corr_matrix[c(5,8,10,9,2,6,4,1),
                                                            c(5,8,10,9,2,6,4,1)], 
       label = FALSE, label_alpha = TRUE, label_size = 3, hjust = 0.95, 
       vjust = 0.3, layout.exp = 9, min_size = 0, max_size = 3.5, 
       limits = c(0,1), low = "#000077", mid = "#EEEEEE", high = "#FF5500")
save_plot("Pairwise_GR50_latitude_orange.svg", plot = last_plot(), base_height = 2.5, base_asp = 1.618)
...
## Visualize mash effects

```{r}
mash_output <- readRDS("FL50/Strong_Effects29475SNPs.rds")
```

```{r}
i=10
get_significant_results(mash_output)[i]
plot1 <- switchgrassGWAS::mash_plot_effects(m = mash_output, n=i)

xlabs <- str_sub(plot1$effect_df$value, start = 11, end = -6)
plot1$effect_df$SITE <- xlabs
  
#plot1$effect_df$SITE <- factor(plot1$effect_df$SITE, levels = c("BRKG", "KBSM", "FRMI", "LINC", "CLMB", "STIL", "OVTN", "TMPL", "PKLE", "KING"))
plot1$effect_df$SITE  <- factor(plot1$effect_df$SITE , levels = c("KING", "PKLE", "TMPL", "OVTN", "STIL", "CLMB", "LINC", "FRMI", "KBSM", "BRKG"))

plot1$ggobject + 
  theme(axis.text.x = element_text(angle = 45, vjust = 1)) + 
  scale_x_discrete(labels = xlabs)

ggplot(data = plot1$effect_df) +
    geom_point(mapping = aes(x = .data$SITE, y = .data$mn)) +
    geom_errorbar(mapping = aes(ymin = .data$mn - .data$se,
                                ymax = .data$mn + .data$se,
                                x = .data$SITE), width = 0.3) +
    geom_hline(yintercept = 0, lty = 2) +
    labs(x = "Conditions", y = "Effect Size") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  coord_flip()

save_plot(paste0(names(get_significant_results(mash_output)[i]), "_Effect_Plot.png"), plot = last_plot(), base_height = 4, base_asp = 0.8)
