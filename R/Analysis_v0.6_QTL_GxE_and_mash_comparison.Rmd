...
for(j in 1:100){
  qtl_size <- qtlplot %>%
    ...
    group_by(CHR, POS_lo) %>%
    mutate(Size = POS_hi - POS_lo) %>%
    select(CHR, POS_lo, Size)
  
  chr_size_mash <- pw_3f_all %>%
    group_by(Chr) %>%
    summarise(Pos_lo = min(Pos),
              Pos_hi = max(Pos))
...
saveRDS(sig_random, file = "../data/Random_QTL_df_significant_enrichments_of_mash_1per_tail_SNPs_100runs.rds")
tibble(`Significant Enrichments` = sig_random) %>%
  ggplot(aes(x = `Significant Enrichments`)) +
  switchgrassGWAS::theme_oeco +
  geom_histogram(binwidth = 1) + 
  xlim(c(1, 24)) + ylab("") +
  geom_vline(xintercept = 23, linetype = 2, color = "red")
save_plot(filename = "../manuscript/plots/Figure_4_QTL_overlaps_random_QTL_enrichments_with_mash.png", plot = last_plot(), base_height = 2)
```
