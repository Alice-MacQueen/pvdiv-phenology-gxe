...
2.  Make hypothesis covariance matrices based on weather data from 2018 - 2020. For cumulative weather variables, use a range of days: every day for the first week, and every week until 28 days. Come back and add more weeks if 28 is chosen by greedy mash.
...
6.  Have Li redo the QTL analysis with any new weather variables and incorporate QTL results.

------------------------------------------------------------------------

## Step 1: GWAS & mash input prep

GWAS were run on BLUPs from models run at each garden individually. The GWAS were run in R/Plots_v0.3_manuscript_figures.Rmd, starting at line 1243. I copy-pasted that info here to have the full workflow in one qmd document; you may need to refer to the Rmd file if you need additional information.

### BLUPs

Copied this code from R/Plots_v0.2_manuscript_figures.Rmd, which is where the ASREML code to calculate BLUPs individually for each location x subpop x phenotype came from; you may need to refer to the Rmd file if you need additional information.
...
NB: only the single_site BLUPs & GWAS that I generated were used in the subsequent analysis.
...
library(here)
txdb <- loadDb(file = here("data-raw", "ignore",
...
I made the mash inputs originally in the Analysis_v0.6_weather_covariance_Uhyp_greenup.Rmd file. I copy-pasted that info here to have the full workflow in one qmd document; some needs to be run on the Juenger lab server.

### Make mashr inputs for 8 GR50 phenotypes & run mash.
...
for(k in c(1, 3)){
phe_suffix <- paste("GR50", snp_v[k], 
                    sep = "*.*")
snp <- snp_attach(inputfiles$SNPfiles[k])
gwas_rds <- pvdiv_results_in_folder(path = here("analysis", "gwas", "BLUPs", "single_sites", "Datatables_single_sites"), 
                                      pattern = paste0("GWAS_datatable*.*",
                                                       phe_suffix))
...
gwas_rds <- pvdiv_results_in_folder(path = here("analysis", "gwas", "BLUPs", "single_sites", "Datatables_single_sites"), 
                                      pattern = paste0("GWAS_datatable*.*",
                                                       phe_suffix))
gwas_rds <- gwas_rds[c(1, 3:4,6:8)]

phenotypes_1 <- str_sub(gwas_rds, start = 16, end = -41) %>%
    str_replace(., "_$", "")
phenotypes <- paste0(names(subpop_v)[k], "_", phenotypes_1)
numSNPs <- round(ceiling(1200000/length(phenotypes)^2)/1000)*1000
out1phe <- pvdiv_bigsnp2mashr(path = here("analysis", "gwas", "BLUPs",
                                          "single_sites",
                                          "Datatables_single_sites"), 
                           snp = snp, gwas_rds = gwas_rds, 
                           phenotypes = phenotypes, numSNPs = numSNPs, 
                           clump = TRUE, scaled = TRUE, saveoutput = TRUE,
                           suffix = paste0("GR50_", names(subpop_v)[k]))
...
Used scp to move these output files (which all have the same suffix) to laptop.

e.g. `scp -i Github/jls_alice alice@popgen.biosci.utexas.edu:~/Github/pvdiv-phenology-gxe/analysis/gwas/BLUPs/single_sites/*19000topSNPs_GR50_Midwest*  /home/alice/Github/pvdiv-phenology-gxe/analysis/gwas/BLUPs/single_sites/Midwest/`
...
Files with the six suffixes I have/want mash results for: B_hat_strong_df_19000topSNPs_FL50_Gulf_and_Midwest.rds B_hat_strong_df_19000topSNPs_FL50_Gulf.rds B_hat_strong_df_19000topSNPs_GR50_Gulf_and_Midwest.rds B_hat_strong_df_19000topSNPs_GR50_Gulf.rds B_hat_strong_df_33000topSNPs_GR50_Midwest.rds B_hat_strong_df_33000topSNPs_FL50_Midwest.rds
...
So I should adjust these to make sure all are positive semi-definite, too, in addition to replacing the diagonals with h^2^.
...
### Midwest covar with fewer sites
...
MO has clear population structure bias in the Manhattan & QQplot

SD dropped because of many 0 Bhat and 0 Shat values

MI, NE, OK, TX1, TX2, TX3
...
for(i in seq_along(phe_hyp)){
    ...
    cov_df <- gr_ten_sites %>%
      ...
      mutate(PLANT_ID = as.factor(PLANT_ID),
             IND = as.factor(IND)) %>%
      select(PLANT_ID, SUBPOP, IND, MI, NE, OK, TX1, TX2, TX3) #####  IL,
    
    cor_phe <- cor(cov_df[,-(1:3)], use = "pairwise")
    # Set diagonal of this matrix = heritability within each garden
    h2_diag <- c()
    site_names <- c("MI", "NE", "OK", "TX1", "TX2", "TX3")
    for(j in seq_along(site_names)) {
      phe_one <- fl_ten_sites %>% filter(manu_site == site_names[j]) %>%
        filter(WHERE != "FWCR") %>%
        group_by(PLANT_ID) %>%
...
#| eval: false
workingdir <- here("analysis", "mash_greedy_algorithm", 
                                   "mash_h2_diag_covar", "midwest_inputs")
pheno = "greenup"
betas = read.table(here(workingdir, "combined_mash_effects", paste0( pheno,'.midwest.betas.random.txt')))
stderrs = read.table(here(workingdir, "combined_mash_stderrs", paste0( pheno,'.midwest.stderrs.random.txt')))
...
When I change to my inputs for effects & stderrs, I will need to adjust midwest covar & effects appropriately to reflect only 6 (flowering) and 6 (greenup) rows & columns used; the other GWAS were dropped as too low power, due to population structure issues in the GWAS, or due to substantial phenotype data loss, & including them would give misleading results, even if it would just result in loadings onto the single garden effect matrices.
...
Try Gulf & All with my known inputs: python run.mash.greedy.search.py -p greenup -c gulf -e random python run.mash.greedy.search.py -p greenup -c all -e random python run.mash.greedy.search.py -p flowering -c gulf -e random
...
for (i in seq_along(popkey)) {
  for (j in seq_along(phekey)) {
    ...
    if (i == 2 & j == 1) {
      betas <- readRDS(file = here("analysis", "gwas", "BLUPs", "single_sites",
                                   "Datatables_single_sites", 
                                 paste0("B_hat_random_df_33000topSNPs_",
                                        phekey[[j]], "_", popkey[[i]], ".rds")))
      stderr <- readRDS(file = here("analysis", "gwas", "BLUPs", "single_sites",
                                   "Datatables_single_sites",  
...
Given that the greedy algorithm, run on Sam's random inputs, however he generated or found those, gives many more covar matrices especially from Uhyp, I wonder if some repetition of this analysis would be valuable - either different random sets of effects, or effects from different years of data. Think on this. I definitely want to use my known inputs, not Sam's, as mine have a traceable history (or anyways are currently traceable).
...
numSNPs_v <- c(19000, 19000, 19000, 19000, 33000, 33000)
...
for (i in 1:6) {  # change this for different scripts
  ...
  for (k in 1:nrow(U_greedy)) {
    ...
    # MI, MO, NE, OK, TX1, TX2, TX3 used for GR50  
      U_single <- U_single %>%
        filter(`...1` %in% c("MI", "NE", "OK", "TX1", "TX2", "TX3")) %>%
        select(`...1`, .data$MI, .data$NE, .data$OK, .data$TX1,
               .data$TX2, .data$TX3)
    } else if (i == 6) {
...
NB: Figure 1 is in Plots_v0.3_manuscript_figures.Rmd
...
```{r}
get_significant_results(m2)
print(length(get_significant_results(m2)))
get_significant_results(m_midw_gr_hyp)
print(length(get_significant_results(m_midw_gr_hyp)))
print(length(get_significant_results(m_midw_fl_hyp)))
print(length(get_significant_results(m_gulf_gr_hyp)))
print(length(get_significant_results(m_gulf_fl_hyp)))
print(length(get_significant_results(m_gam_gr_hyp)))
print(length(get_significant_results(m_gam_fl_hyp)))
```
...
get_greedy_Ulist <- function(i) {
  pop_sc <- c("all", "gulf", "all", "gulf", "midwest", "midwest")
  phe_sc <- c("flowering", "flowering", "greenup", "greenup", 
              "greenup", "flowering")
  Upaths <- c(here("analysis", "mash_greedy_algorithm", "mash_h2_diag_covar",
                   "all_inputs"),
              here("analysis", "mash_greedy_algorithm", "mash_h2_diag_covar",
...
#for (i in 1:6) {  # change this for different scripts
  ...
  mutate(`Covariance Matrix` = str_replace(`Covariance Matrix`, "greenup.", ""),
         ...
         `Covariance Matrix` = str_replace(`Covariance Matrix`, "equal_effects", "Equal Effects"),
         `Covariance Matrix` = str_replace(`Covariance Matrix`, "cgdd_12c_28d.U.mat", "Cumulative GDD (28d)"),
         `Covariance Matrix` = str_replace(`Covariance Matrix`, "cgdd_12c_1d.U.mat", "Cumulative GDD (1d)"),
...
for (i in 1:nrow(U1)) {
  ...
  plot_covar <- U1 %>%
    ...
    scale_fill_gradientn(colors = c("#30123BFF", "#4777EFFF",
                                    "#1BD0D5FF",
...
pa13 <- plot_covar_Ulist_fig2(Ulist_mg[[3]], mg = TRUE) + 
  labs(subtitle = "Equal Effects") 
pa14 <- plot_covar_Ulist_fig2(Ulist_mg[[7]], mg = TRUE) + 
  labs(subtitle = "Cumulative GDD (28d)") 
pa15 <- plot_covar_Ulist_fig2(Ulist_gf[[1]]) + 
...
df_2fl_a$Type <- factor(df_2fl_a$Type, levels = c("Canonical", "GxWeather"))
df_2fl_b$Type <- factor(df_2fl_b$Type, levels = c("Canonical", "GxWeather"))
...
Build Figure 2 with covariance matrices as first two rows over covar barplots

```{r}

ggdraw() +
  theme_oeco + 
  draw_plot(pa01, x = 0, y = 0.66, width = 0.1, height = 0.18) +
  draw_plot(pa14, x = 1/6, y = 0.66, width = 0.1, height = 0.18) +
  #draw_plot(pa03, x = -0.02, y = 0.6, width = 0.1, height = 0.18) +
  #draw_plot(pa04, x = 0.1, y = 0.6, width = 0.1, height = 0.18) +
  draw_plot(pa05, x = 2/6, y = 0.66, width = 0.1, height = 0.18) +
  draw_plot(pa02, x = 0.5, y = 0.66, width = 0.1, height = 0.18) +
  draw_plot(pa06, x = 4/6, y = 0.66, width = 0.1, height = 0.18) +
  draw_plot(pa07, x = 5/6, y = 0.66, width = 0.1, height = 0.18) +
  #draw_plot(pa08, x = 0.2, y = 0, width = 0.1, height = 0.18) +
  #draw_plot(pa09, x = 0, y = 0.83, width = 0.1, height = 0.18) +
  draw_plot(pa10, x = 1/6, y = 0.83, width = 0.1, height = 0.18) +
  draw_plot(pa12, x = 2/6, y = 0.83, width = 0.1, height = 0.18) +
  draw_plot(pa13, x = 0.5, y = 0.83, width = 0.1, height = 0.18) +
  draw_plot(pa11, x = 4/6, y = 0.83, width = 0.1, height = 0.18) +
  draw_plot(pa09, x = 5/6, y = 0.83, width = 0.1, height = 0.18) +
  draw_plot(f2a, x = 0, y = 0.33, width = 0.66, height = 0.38) +
  draw_plot(legend, x = 0.1, y = 0.77, width = 0.1, height = 0.2) +
  draw_plot(f2b, x = 0.66, y = 0.33, width = 0.33, height = 0.38) +
  draw_plot(f2c, x = 0, y = 0, width = 0.66, height = 0.33) +
  draw_plot(f2d, x = 0.66, y = 0, width = 0.33, height = 0.33)
```

```{r}
#| eval: false

save_plot(filename = "../manuscript/revision2/intermediate_files/Figure_2_six_sites_Midwest.svg", plot = last_plot(), base_height = 7.5, base_width = 6.5)
...
one_gxe_summary <- function(df, n_cond = 8, n_sep = 2) {
  ...
  df <- df %>%
    ...
    dplyr::rename("Region_1" = "Region") %>%
    left_join(site_regions, by = c("Garden_2" = "Site")) %>%
    dplyr::rename("Region_2" = "Region") %>%
  ...
  df <- df %>%
    ungroup() %>%
    dplyr::select(-Region_1, -Region_2) %>%
...
get_gxe_summary(gxe_mf, n_cond = 6) 
```

```{r}
fig3_gg <- get_gxe_summary(gxe_gg) 
fig3_mg <- get_gxe_summary(gxe_mg, n_cond = 6) 
...
save_plot(filename = "../manuscript/revision2/intermediate_files/Figure_3_Fraction_GxE_by_Region_six_midwest_sites.svg", base_height = 6, base_width = 4.5, plot = last_plot())
```
...
pvdiv_plot_cor <- function(corrmatrix, upper = NULL, legend = FALSE){
  ...
  U1_covar <- U1 %>%
    ...
    scale_fill_viridis_c(option = "A", limits = c(0, upper)) +
...
```{r}
 pvdiv_plot_cor(corrmatrix = gxe_mf$S_AP, legend = TRUE) + 
  labs(subtitle = " ")
```
...
p3legend_fl <- pvdiv_plot_cor(corrmatrix = gxe_gg$S_AP, legend = TRUE, 
                           upper = 2500) + 
  labs(title = "Gulf", subtitle = "Sign difference")
legend_fl <- get_legend(p3legend_fl)
...
ggdraw() +
  ...
  draw_plot(legend, x = 0.67, y = .68, width = 1/3, height = .18) +
  draw_plot(legend_fl, x = 0.37, y = .38, width = 1/3, height = .18)
...
save_plot(filename = "../manuscript/revision2/intermediate_files/Figure_S3_S4_Effect_Types_by_Garden.svg", plot = last_plot(), base_height = 10, base_width = 5.5)
```
...
quantile(pw_mg$log10BayesFactor, 0.9998446)  # Midwest: Use 2 BF
...
enrich_mg <- get_QTL_enrichment(m_midw_gr_hyp, qtl_df = qtlplot_gr, 0.9998446) %>%
...
save_plot(filename = "../manuscript/revision2/intermediate_files/Figure_4_QTL_Overlaps_6_Midwest_Gardens.svg", base_height = 9, base_width = 6.5, plot = last_plot())
```
