...
save_plot(filename = "../manuscript/Coauthors/PNAS/Revision/Figure_4_QTL_effects_by_trait.png", base_height = 4.5, base_width = 13.5, plot = last_plot())
...
pw_3f <- pw_gf %>%
  ...
  full_join(pw_gamf) %>%
  arrange(Chr, Pos) %>%
  dplyr::select(Chr:Pos, log10BayesFactor, subpop, value, phenotype, everything()) %>%
...
for(k in 1:nrow(f4df)){
  pw_int <- pw_3f %>%
    ...
    arrange(dplyr::desc(log10BayesFactor))
...
save_plot(filename = "../manuscript/Coauthors/PNAS/Revision/Figure_4_mash_SNP_effects.png", plot = last_plot(), base_height = 2.4, base_width = 5.5)
```
