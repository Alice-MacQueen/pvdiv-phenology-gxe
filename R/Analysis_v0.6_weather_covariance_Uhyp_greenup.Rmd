...
# Script used to make figures
# -------------------------
NB: This script uses weather covariance matrices as 
...
```
1.208534e-01+1.400286e-01  photoperiod cues
2.371512e-02  cgdd_12c_gr2fl_Gulf_and_Midwest

5.718063e-02+1.461888e-01+1.398986e-01
...
annos_s1_1 <- annos_gg %>%
  ...
  dplyr::select(where(not_all_na)) %>%
  mutate(Phenotype = "Greenup",
         Subpop = "Gulf")

annos_s1_2 <- annos_gamg %>%
  filter(cgdd_12c_18d_Gulf_and_Midwest.10 > 0.1 |
...
annos_s1_3 <- annos_mf %>%
  ...
  mutate(Phenotype = "Flowering",
         Subpop = "Midwest")
annos_s1_4 <- annos_gamf %>%
...
annos_s1_1 %>%
  full_join(annos_s1_2) %>%
  full_join(annos_s1_3) %>%
  full_join(annos_s1_4) %>%
  ungroup() %>%
  dplyr::select(Marker, Phenotype, Subpop, Chr, Pos, log10BayesFactor, `distance from gene`, `Gene ID`, gene_start, gene_end, gene_width, Pfam:KOG, KO, `Arabidopsis thaliana homolog`:`Rice gene annotation`,
         OsGene, AtGene, Best.hit.arabi.name:rice.defline, leaf:mitochondria, 	
`transcription factor`:`blast disease`, `early leaf senescence`:dormancy, flower:`florigen transport`, everything()) %>%
  dplyr::select(-(CHR:value), -(POS:POS_bin)) %>%
  write_csv("~/Github/pvdiv-phenology-gxe/manuscript/tables/Table_S1_sig_hyp_matrix_SNPs.csv")
```
Not many SNPs with U_hyp loading > 10% and SNP with log10BF > 2:
Both GR: 10 & 9 SNPs   cgdd_12c_18d_Gulf_and_Midwest x2 (0.9 and 0.10)
Both FL: 2 SNPs        dyln_change_sec_Midwest.7
...
save_plot(filename = "../manuscript/plots/Partial/AP_Flowering_Both.png", plot = f3x, base_height = 2, base_width = 2)
save_plot(filename = "../manuscript/plots/Partial/DS_Flowering_Both.png", plot = f3w, base_height = 2, base_width = 2)

ggdraw() +
  draw_plot(f3w, x = 0, y = 0, width = 0.5, height = 6/7) +
  draw_plot(f3x, x = 0.5, y = 0, width = 0.5, height = 6/7) +
  draw_plot(f3w_leg, x = 0.335, y = 0.725, width = 0.1, height = 0.1) +
  draw_label(label = "Sign changes (AP)", x = 0.2, y = 0.92, size = 10) +
  draw_label(label = "Magnitude changes (DS)", x = 0.73, y = 0.92, size = 10)
save_plot(filename = "../manuscript/plots/Partial/AP_DS_Flowering_Both.png", plot = last_plot(), base_height = 2, base_width = 3.5)
```
...
gxe_gamg$S_1_row %>%
  ...
  pivot_longer(cols = 2:9, names_to = "Condition_2", values_to = "Sig") %>%
  separate(Condition_1, into = c(NA, NA, NA, NA, "Phenotype", "Garden_1"), sep = "_") %>%
  separate(Condition_2, into = c(NA, NA, NA, NA, NA, "Garden_2"), sep = "_") %>%
  ...
  summarise(Total_sig = sum(Sig))

gxe_gf$S_AP %>%
  as_tibble(rownames = "Condition_1") %>%
  pivot_longer(cols = 2:9, names_to = "Condition_2", values_to = "Sig") %>%
  separate(Condition_1, into = c(NA, NA, "Phenotype", "Garden_1"), sep = "_") %>%
  separate(Condition_2, into = c(NA, NA, NA, "Garden_2"), sep = "_") %>%
  left_join(site_regions, by = c("Garden_1" = "Site")) %>%
  rename("Region_1" = "Region") %>%
  left_join(site_regions, by = c("Garden_2" = "Site")) %>%
  rename("Region_2" = "Region") %>%
...
gxe_mg$S_2_no %>%
  as_tibble(rownames = "Condition_1") %>%
  pivot_longer(cols = 2:8, names_to = "Condition_2", values_to = "Sig") %>%
  separate(Condition_1, into = c(NA, NA, "Phenotype", "Garden_1"), sep = "_") %>%
  separate(Condition_2, into = c(NA, NA, NA, "Garden_2"), sep = "_") %>%
  left_join(site_regions, by = c("Garden_1" = "Site")) %>%
  rename("Region_1" = "Region") %>%
...
gxe_mf$S_AP %>%
  as_tibble(rownames = "Condition_1") %>%
  pivot_longer(cols = 2:7, names_to = "Condition_2", values_to = "Sig") %>%
  separate(Condition_1, into = c(NA, NA, "Phenotype", "Garden_1"), sep = "_") %>%
  separate(Condition_2, into = c(NA, NA, NA, "Garden_2"), sep = "_") %>%
  left_join(site_regions, by = c("Garden_1" = "Site")) %>%
  rename("Region_1" = "Region") %>%
