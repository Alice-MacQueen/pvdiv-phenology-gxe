...
m_out <- mash_standard_run(path = file.path(outputdir), #, names(subpop_v)[k]), # use to run on genegenie within mash dirs
                             ...
                             suffix = paste0("GR50_", names(subpop_v)[k]),
                             saveoutput = TRUE)
saveRDS(m_out, file = file.path(mashdir, paste0("Strong_Effects_", numSNPs,
                                                "GR50_", names(subpop_v)[k],
                                                "_no_Uhyp.rds")))
m_out1 <- mash_standard_run(path = file.path(outputdir), numSNPs = numSNPs, 
                           U_ed = file.path(outputdir, paste0("Model_of_",
                           "random_tests_24000SNPs_GR50_Midwest.rds")),
                           U_hyp = U_hyp,
                           suffix = paste0("GR50_", names(subpop_v)[k]),
                           saveoutput = FALSE)
saveRDS(m_out1, file = file.path(mashdir, paste0("Strong_Effects_", numSNPs,
                                                "GR50_", names(subpop_v)[k],
                                                "_no_Uhyp.rds")))
```
...
```{r load mash objects}
gulfm <- readRDS("~/Github/pvdiv-phenology-gxe/analysis/mash/Gulf/Strong_Effects19000SNPs_GR50_Gulf_Uhyp.rds")

gandm <- readRDS("~/Github/pvdiv-phenology-gxe/analysis/mash/Gulf_and_Midwest/Strong_Effects19000SNPs_GR50_Gulf_and_Midwest_Uhyp.rds")
```
