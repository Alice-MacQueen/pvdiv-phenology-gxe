...
annos_plot <- annos_df %>%
  filter(sites %in% c("eight_sites", "tx_sites", "north_sites") & !(phe %in% c("GR50", "cgdd_12c_10d"))) %>%
  group_by(CHR, POS_Mb, sites, subpop, phe) %>% add_tally(name = "n_SNPs") %>%
  arrange(CHR, POS_Mb) %>%
  #filter(n_SNPs > 1) %>%
  slice(1) %>%
  group_by(CHR, POS_Mb) %>% add_tally(name = "n_Mb") %>%
  filter(n_Mb > 7) %>% arrange(CHR, POS_Mb) %>%
...
annos_plot1 <- annos_plot %>% group_by(CHR, POS_Mb) %>% tally() %>%
  mutate(POS = POS_Mb*1000000+5000)
annos_plot2 <- annos_plot %>% group_by(CHR, POS_Mb) %>% tally() %>%
  mutate(POS = POS_Mb*1000000) %>% filter(POS_Mb > 50)
...
log10p_long %>%
  filter(subpop == "Midwest" & log10p > 7) %>% group_by(log10p) %>% tally() %>% arrange(desc(n)) # 9.242224 12.466319
...
log10p_plot %>%
  ...
  slice(1)
  summarise(minP = min(POS),
            maxP = max(POS), .groups = "keep")
...
log10p_plot %>%
  ...
  facet_grid(site ~ CHR, scales = "free_x", space = "free_x", switch = "x", ) +
      theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), 
            ...
            panel.background = element_rect(fill = "grey85")) +
            labs(x = "Chromosome", y = "-log10(p value)") +
            scale_x_continuous(breaks = c(20000000, 40000000, 60000000, 80000000, 100000000), labels = c("20 Mb", "40 Mb", "60 Mb", "80 Mb", ""), expand = c(0.15, 0.15)) +
      scale_color_viridis(discrete = TRUE, end = 0.93,
                          option = "B") +
      scale_shape_manual(values = c(6, 4, 1, 4, 16, 4, 17, 18, 14, 3, 4)) +
  ylim(c(6, 15)) + geom_hline(yintercept = c(8.3), linetype = 2) + labs(subtitle = "K subgenome")
save_plot(paste0("../manuscript/plots/", "G_BLUP_Combined_Manhattans_fl_phe_subpops_sites_K_subgenome.png"), 
              base_height = 4, plot = last_plot())
```
