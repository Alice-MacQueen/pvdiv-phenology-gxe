...
for(i in seq_along(subpop_v)){
  ...
  if(names(set_v)[j] != "single_sites"){
    phe_df <- phe_df %>%
    select(PLANT_ID, ends_with(names(set_v)[j]))
  } else {
    phe_df <- phe_df %>%
    select(PLANT_ID, !ends_with("sites"))
  }
...
save_plot("../manuscript/plots/Gulf_G_BLUP_top_500_Manhattan_2.png", base_height = 5, base_asp = 2.5, plot = gulf_top_manh2)

gulf_top_manh <- gulf_top %>%
  filter(max_score_log10p > 5.897) %>%
  ggplot(aes(x = POS, y = max_score_log10p)) + 
  geom_point(aes(color = n_cond_in_top_set, shape = Condition)) + 
  #geom_line(aes(group = Condition)) +
  facet_wrap( ~ CHR, nrow = 1, scales = "free_x", strip.position = "bottom") +
  theme(legend.position = "top",
        axis.text.x = element_blank(), axis.ticks.x = element_blank(),
  ...
  scale_shape_manual(values = c(15, 16, 17, 18, 14, 3, 4))
  
save_plot("../manuscript/plots/Gulf_G_BLUP_top_500_Manhattan.png", base_height = 5, base_asp = 2.5, plot = gulf_top_manh)
```


```{r}
midw_top <- readRDS(file.path(gwasdir, "Part-One-Output_Top-Effects-1000-SNPs_GR_FL_Midwest.rds"))

midw_top %>%
  group_by(n_cond_in_top_set) %>%
  tally()

midw_top_manh2 <- midw_top %>%
  filter(max_score_log10p > 6) %>%
  ggplot(aes(x = POS, y = max_score_log10p)) + 
  geom_point(aes(color = Condition, shape = as_factor(n_cond_in_top_set))) + 
  #geom_line(aes(group = Condition)) +
  facet_wrap( ~ CHR, nrow = 1, scales = "free_x", strip.position = "bottom") +
  ...
  scale_color_viridis(discrete = TRUE, end = 0.97) +
  scale_shape_manual(values = c(4, 16, 17, 18, 14, 3, 4))
save_plot("../manuscript/plots/Midwest_G_BLUP_top_500_Manhattan_2.png", base_height = 5, base_asp = 2.5, plot = midw_top_manh2)

midw_top_manh <- midw_top %>%
  filter(max_score_log10p > 5.897) %>%
  ggplot(aes(x = POS, y = max_score_log10p)) + 
  geom_point(aes(color = n_cond_in_top_set, shape = Condition)) + 
  #geom_line(aes(group = Condition)) +
...
save_plot("../manuscript/plots/Midwest_G_BLUP_top_500_Manhattan.png", base_height = 5, base_asp = 2.5, plot = midw_top_manh)
```

```{r}
gandm_top <- readRDS(file.path(gwasdir, "Part-One-Output_Top-Effects-1000-SNPs_GR_FL_Gulf_and_Midwest.rds"))

gandm_top %>%
  group_by(n_cond_in_top_set) %>%
  tally()

gandm_top_manh2 <- gandm_top %>%
  filter(max_score_log10p > 6) %>%
  ggplot(aes(x = POS, y = max_score_log10p)) + 
  geom_point(aes(color = Condition, shape = as_factor(n_cond_in_top_set))) + 
  #geom_line(aes(group = Condition)) +
  ...
  scale_color_viridis(discrete = TRUE, end = 0.97) +
  scale_shape_manual(values = c(4, 16, 17, 18, 14, 3, 4))
save_plot("../manuscript/plots/Gulf_and_Midwest_G_BLUP_top_500_Manhattan_2.png", base_height = 5, base_asp = 2.5, plot = gandm_top_manh2)

gandm_top_manh <- gandm_top %>%
  filter(max_score_log10p > 5.897) %>%
  ggplot(aes(x = POS, y = max_score_log10p)) + 
  geom_point(aes(color = n_cond_in_top_set, shape = Condition)) + 
  #geom_line(aes(group = Condition)) +
...
save_plot("../manuscript/plots/Gulf_and_Midwest_G_BLUP_top_500_Manhattan.png", base_height = 5, base_asp = 2.5, plot = gandm_top_manh)
```
