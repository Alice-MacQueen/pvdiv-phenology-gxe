...
site_plot <- c("TX1", "TX2", "TX3", "OK", "MO", "NE", "MI", "SD", "All", "Texas", "North")

flh2_p1 <- flh2 %>%
  filter(subpop_name %in% c("Gulf and Midwest", subpop_v[[1]], subpop_v[[2]]) & (site %in% site_plot) & !is.na(site)) %>% # subpop == subpop_v[[i]] & 
...
save_plot("../manuscript/plots/h2_FL_site_weather_functions_All_Sites_v2.png", plot = flh2_p1, base_height = 7, base_asp = 1.3)
...
grh2 <- h2gr %>%
  ...
  mutate(site = case_when(is.na(site) ~ "all",
         ...
         phe_name = case_when(phe == "GR50" ~ "50% greenup",
                              phe == "FL50" ~ "50% flowering",
                              phe == "cgdd_12c_jan2gr" ~ "GDD 01-01 to greenup",
                              ...
                              TRUE ~ phe))
grh2$site <- factor(grh2$site, levels = c("TX1", "TX2", "TX3", "OK", "MO", "NE", "IL", "MI", "SD", "All", "Texas", "North", "nine_sites"))
...
grh2_p1 <- grh2 %>%
  ...
  theme(panel.spacing.x = unit(0.1, 'cm'), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none") + ylim(c(0, 1.1)) +
  scale_fill_viridis(discrete = TRUE, end = 0.8, direction = 1)
save_plot("../manuscript/plots/h2_GR_site_weather_functions_All_Sites_v2.png", plot = grh2_p1, base_height = 7, base_asp = 1.3)
...
grh2_p2 <- grh2 %>%
  ...
  theme(panel.spacing.x = unit(0.1, 'cm'), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none") + ylim(c(0, 0.5)) +
  scale_fill_viridis(discrete = TRUE, end = 0.8, direction = 1)
save_plot("../manuscript/plots/h2_GR_site_weather_functions_All_Sites_v3.png", plot = grh2_p2, base_height = 5, base_asp = 1.5)
...
grh2_p3 <- grh2 %>%
  ...
  facet_grid(site ~ subpop_name) +
  geom_hline(data = filter(grh2, subpop_name %in% c("Gulf and Midwest", subpop_v[[1]], subpop_v[[2]]) & (site %in% site_plot) & !is.na(site) & phe == "GR50"), aes(yintercept = Estimate), linetype = 2) +
  theme(panel.spacing.x = unit(0.1, 'cm'), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none") + ylim(c(0, 0.15)) +
  scale_fill_viridis(discrete = TRUE, end = 0.8, direction = 1) + xlab("")
