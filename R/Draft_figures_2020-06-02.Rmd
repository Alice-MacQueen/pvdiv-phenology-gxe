...
subpops <- read_rds("../data/subpop_color_coding.rds")
phenotypes <- 
...
fwcr_df <- phenotypes %>%
  filter(WHERE == "FWCR" & !(SITE %in% c("OVTN", "FRMI", "MNHT")) & YEAR == 2019) %>%
  unite(col = "SITE_PHE", SITE, PHE) %>%
  select( -WHERE, -SUBPOP, -PLOT_GL) %>%
  pivot_wider(names_from = SITE_PHE, values_from = MEAS, values_fn = mean)

phe_df %>%
  ggplot(aes(x = PKLE_CRAIN, TMPL_CRAIN)) + geom_point(aes(color = SUBPOP)) + geom_abline(slope = 1, linetype = 2) +
  scale_color_manual(values = subpops$color_code)

fwcr_df %>%
  ggplot(aes(x = PKLE_CRAIN, TMPL_CRAIN)) + geom_point() + geom_abline(slope = 1, linetype = 2)

fwcr_df %>%
  filter(PKLE_CGDD_12C > 1200)
fwcr_df %>%
  filter(KBSM_CGDD_12C < 200)

gddcor <- phe_df %>%
  select(matches("CRAIN$")) %>%
  cor(., use = "pairwise")

gddcor %>%
  as_tibble(rownames = "SITE") %>%
  pivot_longer(cols = -SITE, names_to = "SITE2", values_to = "COR") %>%
  ggplot(aes(x = SITE, y = SITE2)) + 
  geom_tile(aes(fill = COR)) +
  scale_fill_viridis(option = "B") +
  theme(legend.position = "right", axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))

phe_df <- phe_df %>% select(PLANT_ID, SUBPOP, matches("GDD"), matches("GR50"), matches("FL50"), matches("CRAIN"), matches("PTT"), everything())
...
```
I've messed up making CGDD for the fourway cross somehow. especially for KBSM. Ugh.

KBSM & LINC most similar. CLMB, BRKG, LINC, and KBSM (and less so STIL) very similar. All corr > 0.6 though.

```{r}
phe_all <- read_rds("../data/older/all_phenotype_meas_4wcr_gwas.rds")
...
We first evaluated the evidence that switchgrass photoperiodicity was genotype dependent – specifically, that switchgrass flowers when exposed to shortening days of a specific length. 
```{r}
fl50_dyln <- phe_all %>%
  filter(PHE == "FL50") %>%
