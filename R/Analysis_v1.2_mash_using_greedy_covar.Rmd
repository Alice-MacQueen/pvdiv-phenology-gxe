...
Some univariate GWAS for the Midwest had population structure issues. Mostly for greenup - even GR50 at MO was quite bad. Drop MO for GR50.
...
fig_eqeff <- cov_gr1$greenup.simple_het_2_df %>%
  get_covar_fig2() +
...
get_gxe_summary <- function(gxe_list, n_cond = 8, n_sep = 2) {
  ...
  gxe_frac <- gxe_n %>%
    mutate(North_North = round(North_North/sum(North_North), digits = 4),
           North_Texas = round(North_Texas/sum(North_Texas), digits = 4),
           Texas_Texas = round(Texas_Texas/sum(Texas_Texas), digits = 4))
...
```{r}
mash_plot_Ulist(m_midw_fl_hyp)
mash_plot_manhattan_by_condition(m_midw_fl_hyp)
```
