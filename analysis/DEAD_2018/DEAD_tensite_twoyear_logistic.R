...
setwd(analysisdir)
dead <- readRDS("DEAD_2018_Logistic_GWAS_df.rds")
...
df <- plants %>%
  ...
  left_join(dead, by = "PLANT_ID") %>%
  dplyr::select(-name)

lambdagc3 <- pvdiv_lambda_GC(df = df, type = "logistic", snp = snp,
                            covar = svd, ncores = NCORES,
                            npcs = c(0:20), saveoutput = TRUE)


PCdf <- switchgrassGWAS:::asv_best_PC_df(lambdagc3)
...
for(i in 1:nrow(PCdf)){
  ...
  if(PCdf1$NumPCs == 0){
    gwas <- pvdiv_gwas(df = df1, type = "logistic", snp = snp, ncores = NCORES)
  } else {
    gwas <- pvdiv_gwas(df = df1, type = "logistic", snp = snp, covar = svd,
  ...
  # Save Manhattan plot with 5% FDR threshold and two colors
  ggmanobject1 <- gwas_data %>%
    ...
    scale_color_manual(values = rep(c("#280B54FF", "grey"), 9), guide = FALSE)
  ...
  anno_tables <- pvdiv_table_topsnps(df = gwas, type = "bigsnp", n = c(10,500),
                                     ...
                                     txdb = txdb)
  saveRDS(anno_tables, file.path(analysisdir,
  ...
  ggsave(snp_qq(gwas[which(!is.na(gwas$score)),]),
         filename = paste0("QQplot_", names(df1)[2], "_", PCdf1$NumPCs,
                           "_PCs.png"), width = 4,
         height = 4, path = file.path(analysisdir))
